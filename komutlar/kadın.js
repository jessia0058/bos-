...
exports.run = async(client, message, args) => {
  console.log(args)
        ...
        let kadınRol = message.guild.roles.get("753272499092848732")
        let kadınRol2 = message.guild.roles.get("753272500065927198")
        let kayıtsız = message.guild.roles.get("753272507036860497")
        if (!veri) {
            db.set(`${message.guild.id}.${member.id}.kayıt`, { cinsiyet: [kadınRol.toString()], isimyas: [`${isim}`] })
        } else {
...
member.removeRole(kayıtsız.id)
    member.addRole(kadınRol.id)
    member.addRole(kadınRol2.id)
